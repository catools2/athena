...
public class TestCaseClient {
  ...
  private static final ScaleTestClient SCALE_TEST_CLIENT =
      getScaleTestsClient(ScaleTestClient.class);

  private final ScaleAtmClient SCALE_ATM_CLIENT = getScaleAtmClient(ScaleAtmClient.class);

  public static void processProjectTestCases(
  ...
  public static void processAllTestCases(
      final int threadsCount,
      final long timeoutInMinutes,
      final String query,
      String fields,
    ...
    executeInParallel(
        ...
        () -> {
          while (true) {
            ...
            log.info(
                "Process TestCases from {} to {} for query {}",
                startFrom,
                startFrom + CoreConfigs.getBufferSize(),
                query);
            Set<ScaleTestCase> scaleTestCases =
                readSectionTestCases(query, fields, startFrom, CoreConfigs.getBufferSize());
  ...
  private static Set<ScaleTestCase> readSectionTestCases(
      final String query, String fields, int startAt, int maxResults) {
    sleep(ScaleConfigs.getDelayBetweenCallsInMilliseconds());
    String homeUri = ScaleConfigs.getAtmUri();
    log.trace(
  ...
  private static Integer _getTestId(final String testCaseKey) {
    ...
    try {
      return JsonPath.read(
          IOUtils.toString(response.body().asInputStream(), StandardCharsets.UTF_8), "$.[0].id");
    } catch (IOException e) {
      throw new RuntimeException(
          "Failed to parse response from client for testcase key " + testCaseKey, e);
    }
  }
}
