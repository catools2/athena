...
public interface ScaleAtmClient {
  ...
  @RequestLine(
      "GET /testrun/search?startAt={startAt}&maxResults={maxResults}&query={query}&fields={fields}")
  Set<ScaleTestRun> searchTestRun(
      @Param("startAt") int startAt,
      @Param("maxResults") int maxResults,
  ...
  @RequestLine(
      "GET /testcase/search?startAt={startAt}&maxResults={maxResults}&query={query}&fields={fields}")
  Set<ScaleTestCase> searchTestCase(
      @Param("startAt") int startAt,
      @Param("maxResults") int maxResults,
      @Param("fields") String fields,
  ...
  ScaleTestCase getTestCase(@Param("key") String key);

  @RequestLine("GET /testrun/{key}")
  ScaleTestRun getTestRun(@Param("key") String key);
