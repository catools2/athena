...
public class KubeLoader {
  ...
  public static void loadNamespaces(
      List<String> namespaces, int totalParallelProcessors, long timeoutInMinutes) {
  ...
  /** Transfer KubePods from common.k8s package and load them to DB */
  public static void loadPods(
      Set<PodDto> pods, Instant lastSync, int totalParallelProcessors, long timeoutInMinutes) {
    ...
    executeInParallel(
        ...
        () -> {
          while (true) {
            ...
            log.info(
                "Process pod {}/{} {} with {} containers",
                counter.getAndIncrement(),
                pods.size(),
                pod.getName(),
                pod.getContainers().size());
