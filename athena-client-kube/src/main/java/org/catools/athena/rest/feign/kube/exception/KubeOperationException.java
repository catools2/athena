...
/**
 * Signals that an attempt to perform operation against the Kubernetes cluster denoted by a
...
public class KubeOperationException extends RuntimeException {
  ...
  public KubeOperationException(final String filename, String message, Throwable cause) {
    super(
        String.format(
            "Failed to perform operation on workbook from %s. message: %s", filename, message),
        cause);
  }
}
