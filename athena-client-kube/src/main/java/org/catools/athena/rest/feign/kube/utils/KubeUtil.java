...
public class KubeUtil {
  ...
  public static Set<PodDto> getNamespacePods(CoreV1Api api, String namespace) {
    int timeout = CoreConfigs.getTimeoutInMinutes().intValue() * 60;
    return getNamespacePods(
        api, namespace, null, null, null, null, null, null, null, null, timeout, null);
  }
   ...
   */
  public static Set<PodDto> getNamespacePods(
      CoreV1Api api,
      String namespace,
      String pretty,
    ...
    try {
      ...
      throw new KubeOperationException(
          String.format(
              ...
              namespace, fieldSelector, labelSelector, resourceVersion, resourceVersionMatch),
          e);
    }
  }
    ...
    if (kubePod.getMetadata() != null) {
      ...
      if (kubePod.getMetadata().getAnnotations() != null) {
        kubePod
            .getMetadata()
            .getAnnotations()
            .forEach((k, v) -> pod.getAnnotations().add(new MetadataDto(k, v)));
      }
      ...
      if (kubePod.getMetadata().getLabels() != null) {
        kubePod
            .getMetadata()
            .getLabels()
            .forEach((k, v) -> pod.getLabels().add(new MetadataDto(k, v)));
      }
    }
  ...
  private static void readPodContainers(PodDto kubePod, V1Pod pod) {
    ...
    if (status.getEphemeralContainerStatuses() != null) {
      kubePod
          .getContainers()
          .addAll(readContainers("ephemeral", status.getEphemeralContainerStatuses()));
    }
  ...
  private static Set<ContainerDto> readContainers(
      final String type, List<V1ContainerStatus> kubeContainers) {
  ...
  private static void readTerminatedStateInfo(
      ContainerDto container, V1ContainerState kubeContainer) {
    ...
    if (StringUtils.isNotBlank(stateTerminated.getMessage())) {
      container
          .getMetadata()
          .add(new MetadataDto("terminated_message", stateTerminated.getMessage()));
    }

    if (StringUtils.isNotBlank(stateTerminated.getReason())) {
      container
          .getMetadata()
          .add(new MetadataDto("terminated_reason", stateTerminated.getReason()));
    }
  }
