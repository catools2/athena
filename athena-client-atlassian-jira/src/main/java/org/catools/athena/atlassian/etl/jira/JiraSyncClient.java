...
public class JiraSyncClient {
  ...
  public void syncJira() {
    syncJira(
        CoreConfigs.getProjectCode(), CoreConfigs.getProjectName(), JiraConfigs.getIssueTypes());
  }
  ...
  public void addItems(
      int threadsCount, long timeoutInMinutes, final String projectKey, String issueType) {

    Date lastSync = TmsClient.getLastSyncInfo(projectKey, SYNC_JIRA, issueType);

    Instant syncStartTime = Instant.now();
    AthenaJiraClient.processIssues(
        threadsCount,
        timeoutInMinutes,
        projectKey,
        issueType,
        ...
        issues -> {
          ...
          TmsClient.saveItem(
              JiraTranslator.translateIssue(issues, projectKey, JiraConfigs.getFieldsToRead()));
        });
