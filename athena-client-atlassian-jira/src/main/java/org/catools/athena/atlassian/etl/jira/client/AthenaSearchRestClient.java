...
import static com.atlassian.jira.rest.client.api.IssueRestClient.Expandos.TRANSITIONS;

// I had to extend some core classes to avid waiting for PR to merge, some PR on client project
// waiting over a year.
public class AthenaSearchRestClient extends AsynchronousSearchRestClient {
  ...
  public AthenaSearchRestClient(final URI baseUri, final HttpClient asyncHttpClient) {
  ...
  public Promise<SearchResult> searchJql(
      ...
      @Nullable
      Set<String> fields) {
    final Iterable<String> expandosValues = Stream.of(SCHEMA, NAMES, CHANGELOG, TRANSITIONS).map(EXPANDO_TO_PARAM).collect(Collectors.toList());
  ...
  private void addOptionalParam(final UriBuilder uriBuilder, final String key, final Object... values) {
