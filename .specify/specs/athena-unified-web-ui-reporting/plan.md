...
- uses a professional reporting/dashboard presentation instead of a placeholder shell
...
- Shared shell, gateway-backed browser ingress, and `core` catalog routes are active.
- `spec` exposes summary, freshness, drift, inventory, and detail workflows in the unified shell.
- `metric` exposes summary, trend, and inventory workflows in the unified shell.
- `tms` now exposes the quality-focused aggregation workspace at `/quality/executions` with summary, trend, inventory, and execution drill-down contracts.
- `pipeline` now exposes the first delivery-focused aggregation workspace at `/pipelines/runs` with summary, trend, and inventory contracts.
- `git` now exposes a repository-freshness workspace at `/git/repositories` with summary, trend, and inventory contracts.
- `kube` now exposes a runtime pod workspace at `/runtime/pods` with summary, trend, and inventory contracts.
- `overview` now consumes live summary and trend signals from `spec`, `git`, `kube`, `metric`, `pipeline`, and `tms`.
- `reports` now has a portfolio route at `/reports/portfolio`, a delivery-and-quality route at `/reports/delivery-quality`, shared in-report navigation, server-backed trend and summary window filtering, report-scoped focus/health filtering, JSON and CSV export, and print-specific stakeholder layouts so the report area behaves like its own workspace.
- `reports` now also includes an executive briefing route aimed at external and leadership audiences, with portfolio coverage and risk-mix visuals built from the same live workspace contracts.
- Browser-facing paged endpoints now use a stable shared page DTO instead of raw `PageImpl` serialization.
- The shell theme has been upgraded toward a more professional reporting console to support external read-heavy users.
- Remaining work focuses on deeper cross-domain report pages, richer workspace-to-report drill-downs, and additional professional visualizations now that the first report surfaces and server-backed report-window contracts are live.
...
Working assumptions to start implementation now:

- treat the first release as spanning all seven user-facing boot services, but start delivery with the shared shell and `core` before any non-core workspace is exposed
- default the first rollout to internal-only and read-first until an explicit external-access decision is recorded
- default deployment to a separate static frontend that is reverse-proxied by `athena-gateway`, because `athena-frontend` currently has no tracked runtime or build files
- require every browser API call to target gateway prefixes only

Concrete execution order from this point:

1. Bootstrap the frontend and gateway foundation so `athena-frontend` becomes a runnable application and the browser has one ingress path.
2. Implement the shared shell plus `core`-driven global filter context and catalog navigation.
3. Close the `metric` readiness backlog before building any metrics workspace pages.
4. Close the `spec` and `pipeline` readiness backlog, then build their first browser pages.
5. Close the `tms` readiness backlog and add the first quality workspace pages.
6. Close the `git` and `kube` readiness backlog and add their first browser-focused workspaces.
7. Add cross-domain dashboards and `/reports/*` only after the domain workspaces above are stable.

Current progress against that order:

1. Frontend and gateway foundation: complete for the current shell.
2. Shared shell plus `core` catalog context: complete for the current shell.
3. `metric` readiness closure and first workspace: complete.
4. `spec` readiness closure and first workspace/detail: complete; `pipeline` readiness closure and first workspace are also complete.
5. `tms` first quality workspace: complete for summary, trend, inventory, and drill-down.
6. `git` and `kube`: complete for the first reporting workspaces.
7. Cross-domain `/reports/*`: started with the first portfolio report route.
...
- create the first runnable browser application baseline without waiting on non-core domain contracts
- establish `athena-gateway` as the only browser ingress for UI assets and API traffic
- prove the shared shell and route structure before any report-heavy page work begins

In scope:

- add the frontend build manifest, toolchain, and test entrypoints in `athena-frontend`
- add the frontend application entrypoint, router bootstrap, shared layout shell, and route placeholders for `/overview` and `/catalog/*`
- add a single browser API client that is hard-wired to gateway prefixes rather than direct service URLs
- add shared loading, error, and empty-state primitives that later domain pages can reuse
- add gateway configuration needed to reverse-proxy or host the frontend and keep API routes intact
- add one smoke test for frontend build and one smoke test for gateway plus UI coexistence

Out of scope:

- real metric, pipeline, tms, spec, git, or kube pages
- browser-side aggregation of report data
- external-user authentication and authorization flows
- saved dashboards, report persistence, or `athena-boot-init` schema work

Deliverables:

- a runnable `athena-frontend` baseline committed to the repo
- a shared app shell with navigation and placeholder routes
- a gateway-only browser API layer
- a documented handoff into Phase 3 for `core` integration

Done criteria:

- `athena-frontend` has a tracked runtime, build command, and test command
- the browser shell can load `/overview` and at least one `/catalog/*` placeholder route
- all frontend API configuration points at `athena-gateway` only
- the gateway can serve or reverse-proxy the frontend without breaking existing service prefixes
- validation exists for frontend build success and gateway-plus-UI smoke behavior
