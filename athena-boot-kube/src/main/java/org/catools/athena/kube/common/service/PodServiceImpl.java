...
public class PodServiceImpl implements PodService {
  ...
  public synchronized <T extends NameValuePair> Set<T> normalizeMetadata(Set<T> metadataSet, MetadataRepository<T> metadataRepository) {
    ...
    for (T md : metadataSet) {
      ...
      try {
        T detachedMd = (T) md.getClass().getDeclaredConstructor().newInstance();
        ...
        T normalizedMd = metadataRepository.findByNameAndValue(detachedMd.getName(), detachedMd.getValue())
            .orElseGet(() -> {
              try {
                return metadataRepository.saveAndFlush(detachedMd);
              } catch (org.springframework.dao.DataIntegrityViolationException e) {
                // Another thread inserted it between our check and save - retry lookup
                return metadataRepository.findByNameAndValue(detachedMd.getName(), detachedMd.getValue())
