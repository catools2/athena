SELECT cycle_short_name, count(*) AS total, sum(pass) AS pass, sum(in_progress) AS in_progress, sum(unexecuted) AS not_executed, sum(fail) AS fail, sum(skipped) AS skipped, sum(blocked) AS blocked
