...
public class GitCommands {
  ...
  public void load(
      ...
      @ShellOption(
              value = {"--username"},
              help = "The username to clone repository",
              defaultValue = ShellOption.NULL)
          String username,
      @ShellOption(
              value = {"--password"},
              help = "The password to clone repository",
              defaultValue = ShellOption.NULL)
          String password,
      @ShellOption(
              value = {"--name"},
      ...
      @ShellOption(
              value = {"--url"},
              help = "The url to the repository to clone project from",
              defaultValue = ShellOption.NULL)
          String url,
      @ShellOption(
              value = {"--repo-info"},
              help =
                  "Set of repositories name and url in json format i.e. [{\"name\": \"...\",\"url\": \"...\"}]",
              defaultValue = ShellOption.NULL)
          String repoInfoSet,
      @ShellOption(
              value = {"--metadata-matcher"},
              help =
                  "Set of metadata name and pattern to read from commit message i.e. [{\"name\": \"...\",\"pattern\": \"...\"}]",
              defaultValue = ShellOption.NULL)
          String metadataPatternSet,
      @ShellOption(
              value = {"--local-path"},
              help = "The path to the local folder where repository should be clone to",
              defaultValue = ShellOption.NULL)
          String localPath,
      @ShellOption(
              value = {"--threads"},
  ...
  private void loadRepository(RepoInfo repoInfo) {
    ...
    RevFilter revFilter =
        savedRepository == null || savedRepository.getLastSync() == null
    ...
    repositoryInfo.uploadRepository(
        CoreConfigs.getThreadsCount(), CoreConfigs.getTimeoutInMinutes(), revFilter);
  }

  private Git cloneRepository(RepoInfo repoInfo) {
    String localPath =
        Path.of(
                StringUtils.defaultIfBlank(GitConfigs.getLocalPath(), "./tmp/repository/"),
                repoInfo.getName() + Instant.now())
            .toFile()
            .getPath();

    if (StringUtils.isNoneBlank(GitConfigs.getUsername())) {
      return GitCloneClient.clone(
          localPath,
          repoInfo.getName(),
          repoInfo.getUrl(),
          GitConfigs.getUsername(),
          GitConfigs.getPassword());
    }
