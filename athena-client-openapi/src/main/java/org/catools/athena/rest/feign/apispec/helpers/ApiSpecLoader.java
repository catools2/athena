...
public class ApiSpecLoader {
  public static final ApiSpecClient API_SPEC_CLIENT =
      getClient(ApiSpecClient.class, CoreConfigs.getAthenaHost());

  public static void saveOpenApi() throws IOException {
    saveOpenApi(OpenApiConfigs.getSpecNames(), OpenApiConfigs.getSpecUrls());
  }

  public static void saveOpenApi(final List<String> specNames, final List<String> urls)
      throws IOException {
    ...
    if (specNames.size() != urls.size()) {
      throw new RuntimeException(
          String.format(
              "Number of spec names and urls does not match! names: %s, urls: %s",
              specNames.size(), urls.size()));
    }
