...
public class PipelineUtils {
  private static final ExecutionStatusClient EXECUTION_STATUS_CLIENT =
      getClient(ExecutionStatusClient.class, CoreConfigs.getAthenaHost());

  public static Optional<PipelineDto> getPipeline(
    ...
    try {
      return Optional.ofNullable(
          getClient(PipelineClient.class, CoreConfigs.getAthenaHost())
              .getPipeline(
                  pipelineName, pipelineNumber, projectCode, versionCode, environmentCode));
    } catch (FeignException.NotFound ex) {
      return Optional.empty();
    }
  }

  public static PipelineExecutionStatusDto getExecutionStatus(
      PipelineExecutionStatusDto executionStatusDto) {
    return Optional.ofNullable(
            EXECUTION_STATUS_CLIENT.getExecutionStatus(executionStatusDto.getName()))
        .orElseGet(
            () -> {
