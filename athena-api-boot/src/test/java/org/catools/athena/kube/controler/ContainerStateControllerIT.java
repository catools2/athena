...
class ContainerStateControllerIT extends KubeControllerIT {
  ...
  void shallReturnCorrectValueWhenValidIdProvided() {
    ResponseEntity<ContainerStateDto> response = containerStateController.getById(CONTAINER_STATE.getId());
  ...
  void getContainerStatesShallReturnCorrectSetWhenValidContainerIdProvided() {
    ResponseEntity<Set<ContainerStateDto>> response = containerStateController.getByContainerId(CONTAINER.getId());
    assertThat(response.getStatusCode().value(), equalTo(200));
    assertThat(response.getBody(), notNullValue());
