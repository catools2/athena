...
create table athena_openapi.api (spec_id bigint not null, first_time_seen TIMESTAMPTZ, id bigserial not null, last_sync_time TIMESTAMPTZ, method varchar(10) not null, url varchar(500) not null, title varchar(1000), description varchar(5000), parameters jsonb, primary key (id));
...
create table athena_openapi.api_spec_metadata_mid (spec_id bigint not null, metadata_id bigint not null, primary key (spec_id, metadata_id));
...
alter table if exists athena_openapi.api add constraint FKdlxwlfd1dp5pymn9pnt7gq9c2 foreign key (spec_id) references athena_openapi.api_spec;
...
alter table if exists athena_openapi.api_spec_metadata_mid add constraint FK9epune7khiuwoye9a4jjlqjjq foreign key (spec_id) references athena_openapi.api_spec;
...
alter table if exists athena_openapi.path_metadata_mid add constraint FKq71fj72q36ap7q7bmpbmf59v9 foreign key (metadata_id) references athena_openapi.api_metadata;
alter table if exists athena_openapi.path_metadata_mid add constraint FK5ilsqmc73evthbqxqln1xhgai foreign key (path_id) references athena_openapi.api;
